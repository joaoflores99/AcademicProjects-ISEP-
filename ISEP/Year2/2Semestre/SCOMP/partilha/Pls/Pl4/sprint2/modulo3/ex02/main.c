...
pid_t pid;
	...
	//pai
		...
		double resPipe = (double)(fimPipe-inicioPipe);
		printf("AO UTILIZAR PIPE DEMOROU %f SEGUNDOS\n",resPipe);
...
//----------------INICIO MEMORIA PARTILHADA----------------//
	...
	if(pid==0){
		...
		double resMem = (double)(fimMem-inicioMem);
		printf("AO UTILIZAR A MEMORIA PARTILHADA DEMOROU %f SEGUNDOS\n ",resMem);
