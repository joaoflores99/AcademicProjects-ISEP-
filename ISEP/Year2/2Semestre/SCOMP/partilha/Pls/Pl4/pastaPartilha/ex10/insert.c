...
int main(void) {
    ...
    dados_introduzidos.nome[tamanho-1] = '\0'; // remover o /n que o fgets captura
    printf("Introduza a sua morada: \n");
    tamanho = strlen(dados_introduzidos.morada);
    dados_introduzidos.morada[tamanho-1] = '\0'; // remover o /n que o fgets captura
    fgets(dados_introduzidos.morada,STRING_SIZE,stdin);
    printf("Introduza o seu número: \n");
    scanf("%d", &dados_introduzidos.numero);
    
    sem_wait(sem_excl); //garantir que a operação de inserção é atómica
