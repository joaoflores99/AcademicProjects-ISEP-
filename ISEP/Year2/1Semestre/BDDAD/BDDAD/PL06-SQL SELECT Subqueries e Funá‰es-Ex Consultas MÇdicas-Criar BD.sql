...
--1
Select M.nome, NVL((Select M2.Nome From medicos M2 where M2.id_medico=M.id_medico_chefe),'')
AS Medico_Chefe From Medicos M order by M.nome;

--2
With consultas_especialidades As 
   (Select e.id_especialidade, e.designacao, Trunc(c.data_hora) data, Count(*) cnt
From consultas C, medicos m, especialidades e Where c.id_medico = m.id_medico And m.id_especialidade = e.id_especialidade
Group By e.id_especialidade, e.designacao, Trunc(c.data_hora)) 
   Select id_especialidade, designacao, data
            ...
            Where cnt = (Select Max(cnt) From consultas_especialidades ce2
...
--3
Select Distinct P.nome From pacientes P Where (Select Count(M.id_medico) From medicos M Where Upper(M.hospital) Like 'PEDRO HISPANO')
= (Select Count(Distinct M1.id_medico) From medicos M1 Inner Join consultas C On C.id_medico = M1.id_medico Where
Upper(M1.hospital) Like 'PEDRO HISPANO' And C.id_paciente = P.id_paciente);
