...
public class GraphAlgorithmsTest {
    ...
    public void testDFS() {
        ...
        assertTrue("Should be null if vertex does not exist", GraphAlgorithmsMap.DFS(completeMap, "LX") == null);

        path = GraphAlgorithmsMap.DFS(incompleteMap, "Faro");
        ...
        path = GraphAlgorithmsMap.DFS(completeMap, "Porto");
        ...
        path = GraphAlgorithmsMap.DFS(incompleteMap, "Viseu");
    ...
    public void testBFS() {
        ...
        assertTrue("Should be null if vertex does not exist", GraphAlgorithmsMap.BFS(completeMap, "LX") == null);

        path = GraphAlgorithmsMap.BFS(incompleteMap, "Faro");
        ...
        path = GraphAlgorithmsMap.BFS(completeMap, "Porto");
        ...
        path = GraphAlgorithmsMap.BFS(incompleteMap, "Viseu");
    ...
    public void testAllPaths() {
        ...
        assertFalse("Should be false if vertex does not exist",
                GraphAlgorithmsMap.allPaths(completeMap, "Porto", "LX", paths));

        assertTrue("There should be paths between Porto and Lisboa in the map",
                GraphAlgorithmsMap.allPaths(incompleteMap, "Porto", "Lisboa", paths));
        ...
        GraphAlgorithmsMap.allPaths(incompleteMap, "Porto", "Faro", paths);
    ...
    public void testTransitiveClosure() {
        AdjacencyMatrixGraph<String, String> m = new AdjacencyMatrixGraph<>();

        m.insertVertex("Porto");
        m.insertVertex("Braga");
        m.insertVertex("VR");
        m.insertVertex("Aveiro");
        m.insertEdge("Porto", "Aveiro", "A1");
        m.insertEdge("Porto", "Braga", "A3");

        AdjacencyMatrixGraph<String, String> exp = new AdjacencyMatrixGraph<>(m.numVertices);

        String dummyEdge = "A";

        exp.insertEdge("Porto", "Braga", dummyEdge);
        exp.insertEdge("Porto", "Aveiro", dummyEdge);
        exp.insertEdge("Braga", "Porto", dummyEdge);
        exp.insertEdge("Braga", "Aveiro", dummyEdge);
        exp.insertEdge("Aveiro", "Porto", dummyEdge);
        exp.insertEdge("Aveiro", "Braga", dummyEdge);

        for (int i = 0; i < exp.numVertices; i++) {
            for (int j = 0; j < exp.numVertices; j++) {
                assertEquals(GraphAlgorithmsMap.transitiveClosure(m, dummyEdge).getEdge(exp.vertices.get(i), exp.vertices.get(j)), exp.getEdge(exp.vertices.get(i), exp.vertices.get(j)));
            }
        }
    }
}
